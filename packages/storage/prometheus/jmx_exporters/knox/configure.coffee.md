...
## Identities
      ...
      options.hdfs_user ?= merge service.deps.knox.options.user
      options.hdfs_group ?= merge service.deps.knox.options.group

## Configuration Layout
configure JMX Exporter to scrape HADOOP Knox metrics.

      options.conf_dir ?= "/etc/prometheus-exporter-jmx/conf"
      options.java_home ?= service.deps.java.options.java_home
      options.conf_file ?= "#{options.conf_dir}/knox.yaml"
...
com.sun.management.jmxremote.ssl.config.file=<file>.
      ...
      options.jmx_config_file ?= "/etc/knox/conf/jmx.properties"
      service.deps.knox.options.env.app_mem_opts += " -Dcom.sun.management.config.file=#{options.jmx_config_file} "
      options.jmx_config ?= {}
      options.jmx_config['com.sun.management.jmxremote'] ?= 'true'
      options.jmx_config['com.sun.management.jmxremote.port'] ?= '9018'
      options.jmx_config['com.sun.management.jmxremote.ssl.config.file'] ?= "/etc/knox/conf/jmx-ssl.properties"

## Enable JMX SSL

      options.ssl = merge service.deps.ssl, service.deps.knox.options.ssl
      if !!options.ssl
        ...
        service.deps.knox.options.env.app_mem_opts += " -Dcom.sun.management.jmxremote.ssl.need.client.auth=false "
        service.deps.knox.options.env.app_mem_opts += " -Dcom.sun.management.jmxremote.ssl=true "
        
        options.jmx_ssl_config['javax.net.ssl.keyStore'] ?= options.ssl.keystore.target
        throw Error 'Missing Knox Keystore Password' unless options.ssl?.keystore?.password
        ...
        throw Error 'Missing Knox Truststore Password' unless options.ssl?.truststore?.password
...
## Enable JMX Authentication
      ...
      if options.authenticate
        ...
        options.jmx_auth_file ?=  '/etc/security/jmxPasswords/knox.password'
...
configure JMX Exporter to scrape HADOOP Knox metrics.
      ...
      options.config['hostPort'] ?= "#{service.deps.knox.node.fqdn}:#{options.jmx_config['com.sun.management.jmxremote.port']}"
...
Note: cluster name shoul not contain other character than ([a-zA-Z0-9\-\_]*)
      ...
      for srv in service.deps.prometheus_monitor
        ...
        for conf in srv.options.config['scrape_configs']
          scrape_config = conf if conf.job_name is "#{options.cluster_name}.knox"
        exist = scrape_config?
        scrape_config ?=
          job_name: "#{options.cluster_name}.knox"
          static_configs:
            [
              targets: []
            ]
