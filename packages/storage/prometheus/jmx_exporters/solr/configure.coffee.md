...
## Identities
      ...
      options.hdfs_user ?= merge service.deps.solr.options.user
      options.hdfs_group ?= merge service.deps.solr.options.group

## Configuration Layout
configure JMX Exporter to scrape HADOOP solr metrics.

      options.conf_dir ?= "/etc/prometheus-exporter-jmx/conf"
      options.java_home ?= service.deps.java.options.java_home
      options.conf_file ?= "#{options.conf_dir}/solr.yaml"
...
com.sun.management.jmxremote.ssl.config.file=<file>.
      ...
      options.jmx_config_file ?= "/etc/solr/conf/jmx.properties"
      service.deps.solr.options.env.app_mem_opts += " -Dcom.sun.management.config.file=#{options.jmx_config_file} "
      options.jmx_config ?= {}
      options.jmx_config['com.sun.management.jmxremote'] ?= 'true'
      options.jmx_config['com.sun.management.jmxremote.port'] ?= '18983'
      options.jmx_config['com.sun.management.jmxremote.ssl.config.file'] ?= "/etc/solr/conf/jmx-ssl.properties"

## Enable JMX SSL

      options.ssl = merge service.deps.ssl, service.deps.solr.options.ssl
      if !!options.ssl
        options.jmx_ssl_file ?= options.jmx_config['com.sun.management.jmxremote.ssl.config.file']
        options.jmx_ssl_config ?= {}
        # service.deps.hdfs_dn.options.opts.java_properties['] ?= ''
        service.deps.solr.options.env.app_mem_opts += " -Dcom.sun.management.jmxremote.ssl.need.client.auth=false "
        service.deps.solr.options.env.app_mem_opts += " -Dcom.sun.management.jmxremote.ssl=true "
        
        options.jmx_ssl_config['javax.net.ssl.keyStore'] ?= options.ssl.keystore.target
        throw Error 'Missing solr Keystore Password' unless options.ssl?.keystore?.password
        ...
        throw Error 'Missing solr Truststore Password' unless options.ssl?.truststore?.password
...
## Enable JMX Authentication
      ...
      if options.authenticate
        ...
        options.jmx_auth_file ?=  '/etc/security/jmxPasswords/solr.password'
...
configure JMX Exporter to scrape HADOOP solr metrics.
      ...
      options.config['hostPort'] ?= "#{service.deps.solr.node.fqdn}:#{options.jmx_config['com.sun.management.jmxremote.port']}"
...
Note: cluster name shoul not contain other character than ([a-zA-Z0-9\-\_]*)
      ...
      for srv in service.deps.prometheus_monitor
        ...
        for conf in srv.options.config['scrape_configs']
          scrape_config = conf if conf.job_name is "#{options.cluster_name}.solr"
        exist = scrape_config?
        scrape_config ?=
          job_name: "#{options.cluster_name}.solr"
          static_configs:
            [
              targets: []
            ]
