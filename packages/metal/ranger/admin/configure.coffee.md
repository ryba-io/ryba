...
- Solr Standalone `@rybajs/metal/solr/standalone`
  Ryba default. You need to set `@rybajs/metal/solr/standalone` on one host.
- Solr Standalone embedded
  No need to have `@rybajs/metal/solr/standalone` on one host, Solr will be installed on the same host as Ranger Admin.
  Change property `solr_type` to `embedded` to use it.
- Solr Cloud `@rybajs/metal/solr/cloud`
  Changes  property `solr_type` to `cloud` and deploy `@rybajs/metal/solr/cloud`
  module on at least one host.
- Solr Cloud on docker `@rybajs/metal/solr/cloud_docker`
  Changes  property `solr_type` to `cloud_docker`.
  Important:
    For this to work you need to deploy `@rybajs/metal/solr/cloud_docker` module on at least on host.
...
And it is configured by Ryba only in @rybajs/metal/solr/cloud_docker installation.

If no `@rybajs/metal/solr/*` is configured Ranger admin deploys a `@rybajs/metal/solr/standalone` 
on the same host than `@rybajs/metal/ranger/admin` module.
