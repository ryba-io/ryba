...
executing this module.
    ...
    export default header: 'Ambari Standalone Install', handler: ({options}) ->
...
Start the service or restart it if there were any changes.
      ...
      @service
        ...
        if: -> @status()
      @call '@rybajs/metal/ambari/server/wait', options.wait, if: -> @status()
...
## Dependencies
    ...
    {properties} = require '@nikitajs/core/lib/file/properties'
