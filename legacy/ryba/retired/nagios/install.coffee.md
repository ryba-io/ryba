...
## Services
        ...
        # HDFS NameNode
        nn_hosts = @hosts_with_module '@rybajs/metal/hadoop/hdfs_nn'
        ...
        [snn_ctx] = @contexts '@rybajs/metal/hadoop/hdfs_snn'#, require('../hadoop/hdfs_snn').configure
        # YARN ResourceManager
        rm_ctxs = @contexts '@rybajs/metal/hadoop/yarn_rm'#, require('../hadoop/yarn_rm').configure
        ...
        # YARN NodeManager
        nm_ctxs = @contexts '@rybajs/metal/hadoop/yarn_nm'#, require('../hadoop/yarn_nm').configure
        if nm_ctxs.length
        ...
        jhs_ctxs = @contexts '@rybajs/metal/hadoop/mapred_jhs'#, require('../hadoop/mapred_jhs').configure
        if jhs_ctxs.length
          hs_webapp_port = jhs_ctxs[0].config.ryba.mapred.site['mapreduce.jobhistory.webapp.address'].split(':')[1]
        # HDFS JournalNodes
        jn_ctxs = @contexts '@rybajs/metal/hadoop/hdfs_jn'#, require('../hadoop/hdfs_jn').configure
        if jn_ctxs.length
          journalnode_port = jn_ctxs[0].config.ryba.hdfs.site["dfs.journalnode.#{protocol}-address"].split(':')[1]
        # HDFS Datanodes
        [dn_ctx] = @contexts '@rybajs/metal/hadoop/hdfs_dn'#, require('../hadoop/hdfs_dn').configure
        ...
        # HBase
        hm_ctxs = @contexts '@rybajs/metal/hbase/master', require('../hbase/master/configure').handler
        hrs_ctxs = @contexts '@rybajs/metal/hbase/regionserver', require('../hbase/regionserver/configure').handler
        # Hive
        hcat_ctxs = @contexts '@rybajs/metal/hive/hcatalog'#, require('../hive/hcatalog').configure
        hs2_ctxs = @contexts '@rybajs/metal/hive/server2'#, require('../hive/server2').configure
        ...
        @file.render
          ...
          context:
            ...
            dfs_ha_enabled: not @host_with_module '@rybajs/metal/hadoop/hdfs_snn'
